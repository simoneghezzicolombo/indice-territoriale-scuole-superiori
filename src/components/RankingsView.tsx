...
import { metricsFor, statusLabel, strongestMetricLabel, ZONE_COPY } from "../i18n";
import { CityData, Language, MetricKey, ViewType } from "../types";
...
const RANKINGS_COPY = {
  it: {
    ...
    export: "Esporta CSV",
    search: "Filtra per comune, provincia o regione",
    noRows: "Nessun comune corrispondente ai filtri.",
    municipality: "Comune",
    province: "Prov.",
    region: "Regione",
    delta: "Scostamento",
    level: "Livello",
    actions: "Azioni",
    compareTitle: "Confronta comune",
    openTitle: "Apri scheda comune",
    csvDownloaded: "CSV scaricato",
    csvText: "Contiene i comuni filtrati e la dimensione selezionata.",
    csvHeaders: "Rank finale,Comune,Provincia,Regione",
    csvDelta: "Scostamento",
    csvLevel: "Livello",
    csvStrength: "Punto di forza",
  },
  en: {
    title: "National rankings",
    intro:
      "Ranking of municipalities covered by the index. You can sort by final score or by each individual dimension.",
    export: "Export CSV",
    search: "Filter by municipality, province or region",
    noRows: "No municipality matches the filters.",
    ...
    level: "Level",
    actions: "Actions",
    compareTitle: "Compare municipality",
    openTitle: "Open municipality card",
    csvDownloaded: "CSV downloaded",
    csvText: "Includes the filtered municipalities and the selected dimension.",
    csvHeaders: "Final rank,Municipality,Province,Region",
    csvDelta: "Change",
    csvLevel: "Level",
    csvStrength: "Strongest point",
  },
} as const;
...
export default function RankingsView({ language, setView, setSelectedCompareCity, openExplorerCity, citiesData }: RankingsViewProps) {
  const copy = RANKINGS_COPY[language];
  ...
  const triggerCsvDownload = () => {
    const metricLabel = metrics.find((metric) => metric.id === activeMetric)?.label || copy.title;
    const headers = `${copy.csvHeaders},${metricLabel},${copy.csvDelta},${copy.csvLevel},${copy.csvStrength}\n`;
    const rows = processedData.map((city) => {
      return `${city.rank},"${city.name}","${city.provinceCode}","${city.region}",${metricValue(city, activeMetric).toFixed(2)},${metricDelta(city, activeMetric)},"${statusLabel(city.status, language)}","${strongestMetricLabel(city, language)}"`;
    }).join("\n");
    ...
    <div className="space-y-8 animate-entrance">
        ...
        <div className="flex flex-col lg:flex-row gap-4 justify-between">
          <div className="flex flex-wrap gap-1">
            {(["ALL", "NORD", "CENTRO", "SUD"] as const).map((zone) => {
              const labels = ZONE_COPY[language];
            ...
            <thead>
              <tr className="bg-[#f2fbff] border-b border-[#bdc9c7] font-mono text-[10px] text-[#3e4947] uppercase tracking-wider">
                ...
                <th className="py-4 px-4 font-bold cursor-pointer hover:text-[#00605b]" onClick={() => handleSort("name")}>
                  {copy.municipality} <ArrowUpDown className="w-3 h-3 inline-block ml-1" />
                </th>
                <th className="py-4 px-4 text-center font-bold w-24">{copy.province}</th>
                <th className="py-4 px-4 font-bold cursor-pointer hover:text-[#00605b] hidden md:table-cell" onClick={() => handleSort("region")}>
                  {copy.region} <ArrowUpDown className="w-3 h-3 inline-block ml-1" />
                </th>
                ...
                <th className="py-4 px-4 font-bold text-right hidden lg:table-cell">{copy.delta}</th>
                <th className="py-4 px-4 font-bold text-center hidden lg:table-cell w-36">{copy.level}</th>
                <th className="py-4 px-4 text-center w-28">{copy.actions}</th>
              </tr>
            </thead>
                    ...
                    <tr key={city.id} className="hover:bg-[#f2fbff]/50 transition-colors group align-middle font-sans text-xs text-[#031f27]">
                      ...
                      <td className="py-3 px-4 text-center hidden lg:table-cell">
                        <span className={`inline-block px-2.5 py-1 rounded-full text-[9px] font-mono font-bold uppercase border ${getStatusBadge(city.status)}`}>
                          {statusLabel(city.status, language)}
                        </span>
                      </td>
      ...
      {showToast && (
        <div className="fixed bottom-6 right-6 bg-[#031f27] text-white p-4 rounded-xl shadow-2xl border border-[#bdc9c7]/30 z-[60] animate-toast-slide">
          <p className="font-sans text-xs font-bold text-white">{copy.csvDownloaded}</p>
          <p className="font-sans text-[11px] text-[#bdc9c7] mt-0.5">{copy.csvText}</p>
        </div>
      )}
    </div>
  );
